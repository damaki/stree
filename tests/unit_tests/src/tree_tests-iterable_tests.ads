...
--  This package tests the Iterable aspect on the Tree type.

package Tree_Tests.Iterable_Tests is

   type Test_Fixture is new AUnit.Test_Fixtures.Test_Fixture with null record;

   procedure Test_Iterable_Empty (T : in out Test_Fixture);
   procedure Test_Iterable_Valid_Tree (T : in out Test_Fixture);
